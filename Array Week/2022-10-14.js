...
const Game = () => {
    ...
    return JSON.stringify(arr1) === JSON.stringify(arr2) ? setIsWin(true) : setIsWin(false)
}
  ...
  function moveBlankLeft(curGrid, blankChar) {
    i = curGrid.indexOf(blankChar)
    if (i === 0) {
        return curGrid;
    } else {
        curGrid.splice(i - 1, 0, curGrid.splice(i, 1)[0])
  ...
  function moveBlankUp(curGrid, blankGrid) {
    const row = curGrid.findIndex(row => row.includes(blankGrid));
    const col = curGrid[row].indexOf(blankGrid);
    if (row === 0) {
        return curGrid;
    } else {
        let temp = curGrid[row][col];
        curGrid[row][col] = curGrid[row-1][col];
        curGrid[row-1][col] = temp;
      checkWin(gridWithBlank, curGrid)
        return curGrid;
    }
}
